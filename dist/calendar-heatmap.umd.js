(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@angular/core'), require('moment'), require('d3')) :
    typeof define === 'function' && define.amd ? define(['exports', '@angular/core', 'moment', 'd3'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.CalendarHeatmap = {}, global._angular_core, global.moment, global.d3));
})(this, (function (exports, i0, moment, d3) { 'use strict';
    ...
    })(exports.OverviewType || (exports.OverviewType = {}));
    var CalendarHeatmap = /** @class */ (function () {
        function CalendarHeatmap() {
            var _this = this;
            this.color = '#ff4500';
            this.overview = exports.OverviewType.global;
            /**
            ...
            */
            ...
            // Overview defaults
            this.history = [exports.OverviewType.global];
            this.selected = {};
        }
        /**
        ...
        CalendarHeatmap.prototype.ngAfterViewInit = function () {
            ...
            this.svg = d3__namespace.select(element)
                .append('svg')
                .attr('class', 'svg');
            ...
            this.tooltip = d3__namespace.select(element).append('div')
        ...
        CalendarHeatmap.prototype.drawChart = function () {
            ...
            switch (this.overview) {
                case exports.OverviewType.global:
                    ...
                    break;
                case exports.OverviewType.year:
                    ...
                    break;
                case exports.OverviewType.month:
                    ...
                    break;
                case exports.OverviewType.week:
        ...
        CalendarHeatmap.prototype.drawGlobalOverview = function () {
            ...
            var data = this.data;
            var year_data = d3__namespace.timeYears(start, end).map(function (d) {
                var date = moment(d);
                return {
                    'date': d,
            ...
            var max_value = d3__namespace.max(year_data, function (d) {
                return d.total;
            });
            // Define year labels and axis
            var year_labels = d3__namespace.timeYears(start, end).map(function (d) {
                return moment(d);
            });
            var yearScale = d3__namespace.scaleBand()
            ...
            this.items.selectAll('.item-block-year')
                ...
                var color = d3__namespace.scaleLinear()
                ...
                // Redraw the chart
                _this.overview = exports.OverviewType.year;
                _this.drawChart();
            })
                .style('opacity', 0)
        ...
        CalendarHeatmap.prototype.drawYearOverview = function () {
            ...
            this.items.selectAll('.item-circle')
                ...
                // Redraw the chart
                _this.overview = exports.OverviewType.day;
                _this.drawChart();
            })
                ...
                // Pulsating animation
                var circle = d3__namespace.select(event.currentTarget);
                var repeat = function () {
                    circle.transition()
                        .duration(_this.transition_duration)
                        .ease(d3__namespace.easeLinear)
                ...
                d3__namespace.select(event.currentTarget).transition()
                    .duration(_this.transition_duration / 2)
                    .ease(d3__namespace.easeLinear)
                    .attr('x', function (d) {
                    return _this.calcItemX(d, start_of_year) + (_this.item_size - _this.calcItemSize(d, max_value)) / 2;
            ...
            // Add month labels
            var month_labels = d3__namespace.timeMonths(start_of_year.toDate(), end_of_year.toDate());
            var monthScale = d3__namespace.scaleLinear()
                .range([0, this.width])
            ...
            this.labels.selectAll('.label-month')
                ...
                _this.removeYearOverview();
                // Redraw the chart
                _this.overview = exports.OverviewType.month;
                _this.drawChart();
            });
            // Add day labels
            var day_labels = d3__namespace.timeDays(moment().startOf('week').toDate(), moment().endOf('week').toDate());
            var dayScale = d3__namespace.scaleBand()
        ...
        CalendarHeatmap.prototype.drawMonthOverview = function () {
            ...
            var max_value = d3__namespace.max(month_data, function (d) {
                return d3__namespace.max(d.summary, function (d) {
                    return +d.value;
                });
            });
            ...
            var weekScale = d3__namespace.scaleBand()
            ...
            var itemScale = d3__namespace.scaleLinear()
            ...
            item_block.selectAll('.item-block-rect')
                ...
                .attr('x', function (d) {
                var total = parseInt(d3__namespace.select(this.parentNode).attr('total'));
                var offset = parseInt(d3__namespace.select(this.parentNode).attr('offset'));
                itemScale.domain([0, total]);
                d3__namespace.select(this.parentNode).attr('offset', offset + itemScale(d.value));
                return offset;
            })
                .attr('width', function (d) {
                var total = parseInt(d3__namespace.select(this.parentNode).attr('total'));
                ...
                var date = new Date(d3__namespace.select(event.currentTarget.parentNode).attr('date'));
                // Construct tooltip
            ...
            this.labels.selectAll('.label-week')
                ...
                _this.removeMonthOverview();
                // Redraw the chart
                _this.overview = exports.OverviewType.week;
                _this.drawChart();
            });
            // Add day labels
            this.labels.selectAll('.label-day').remove();
        ...
        CalendarHeatmap.prototype.drawWeekOverview = function () {
            ...
            var max_value = d3__namespace.max(week_data, function (d) {
                return d3__namespace.max(d.summary, function (d) {
                    return +d.value;
                });
            });
            ...
            item_block.selectAll('.item-block-rect')
                ...
                // Calculate tooltip position
                var total = parseInt(d3__namespace.select(event.currentTarget.parentNode).attr('total'));
                itemScale.domain([0, total]);
                var x = parseInt(d3__namespace.select(event.currentTarget).attr('x')) + _this.tooltip_padding * 5;
                while (_this.width - x < (_this.tooltip_width + _this.tooltip_padding * 3)) {
                    x -= 10;
                }
                var y = dayScale(moment(date).weekday().toString()) + _this.tooltip_padding;
        ...
        CalendarHeatmap.prototype.drawDayOverview = function () {
            ...
            var projectScale = d3__namespace.scaleBand()
                .rangeRound([this.label_padding, this.height])
                .domain(project_labels);
            var itemScale = d3__namespace.scaleTime()
            ...
            this.items.selectAll('.item-block')
                ...
                .attr('width', function (d) {
                var end = itemScale(d3__namespace.timeSecond.offset(moment(d.date).toDate(), d.value));
            ...
            // Add time labels
            var timeLabels = d3__namespace.timeHours(moment(this.selected.date).startOf('day').toDate(), moment(this.selected.date).endOf('day').toDate());
            var timeScale = d3__namespace.scaleTime()
                .range([this.label_padding * 2, this.width])
            ...
            this.labels.selectAll('.label-project')
                ...
                .each(function (d, i) {
                var obj = d3__namespace.select(this), text_length = obj.node().getComputedTextLength(), text = obj.text();
        ...
        CalendarHeatmap.ɵcmp = /*@__PURE__*/ i0__namespace.ɵɵdefineComponent({ type: CalendarHeatmap, selectors: [["calendar-heatmap"]], viewQuery: function CalendarHeatmap_Query(rf, ctx) { if (rf & 1) {
    ...
    (function () { (typeof ngDevMode === "undefined" || ngDevMode) && i0__namespace.ɵsetClassMetadata(CalendarHeatmap, [{
            type: i0.Component,
